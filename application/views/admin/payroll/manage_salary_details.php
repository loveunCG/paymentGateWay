                                    ...
                                    <div class="form-group" id="border-none">
                                        ...
                                        <div class="col-sm-5">
                                            ...
                                            <select name="payment_frequency" id="payment_frequency" class="form-control" required onchange="updatePaymentFrequency(this.value)">
                                                ...
                                                <option value="0" <?php
                                                if ((isset($emp_salary->payment_frequency) && $emp_salary->payment_frequency !='' && $emp_salary->payment_frequency == 0) || ($databasePaymentFrequency==3)) {
                                                    echo   'selected';
                                                }
                                                ?>>Two Weekly</option>
			...
			/*************@pank for calculate taxable gross salary 17-8-2016 start here ***************/
				...
				{//@pank 3-8-2016 for pay tex Start
					...
					{//@pank 3-8-2016 for pay tex END
						...
						$('#payroll_tax_deduction_hidden').val(aData.pt.employeePayTax); //26august 2016
					}
					else
					{
...
payrollCalc();
	...
	function updatePaymentFrequency(value){
		...
		var databasePaymentFrequency ='<?php echo (!empty($emp_salary->basic_salary)) ?  $emp_salary->payment_frequency : 3 ;?>';    
		...
		*/
		   if(defaultBasicSalary=='') 			
			{ 
				...
				///Two weekly 
				if(current_payment_frequency==3 && value==0){					 
						  dynamicBasicSalary= dynamicBasicSalary/2;					  
				} 
				if(current_payment_frequency==1 && value==0){					  
						  dynamicBasicSalary= dynamicBasicSalary*2;					
				}
				if(current_payment_frequency==0 && value==3){					   
						  dynamicBasicSalary= dynamicBasicSalary*2;					  
				}   
				if(current_payment_frequency==0 && value==1){					  
						  dynamicBasicSalary= dynamicBasicSalary/2;					  
				}
